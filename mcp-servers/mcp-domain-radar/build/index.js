...
if (!WHOISXML_API_KEY) {
    console.error("ERROR: WHOISXML_API_KEY environment variable is required.\n" +
...
// =========================================================================
server.tool("find_new_registrations", "Search for newly registered domains matching keywords. Uses WhoisXML Newly Registered Domains API. Default: last 7 days, .com/.net/.org TLDs.", {
    keywords: z
        .array(z.string())
...
// Tool 2: whois_lookup
// =========================================================================
server.tool("whois_lookup", "Full WHOIS lookup for a domain. Returns registrant info, creation/expiration dates, name servers, and registrar.", {
...
// =========================================================================
server.tool("monitor_domains", "Monitor a list of domains for WHOIS changes since last check. Tracks registrant, name server, and registrar changes.", {
    domains: z
        .array(z.string())
    ...
    for (const domain of domains) {
        try {
            ...
            if (!previous) {
                ...
                results[results.length - 1].first_check = true;
            }
        }
        catch (error) {
            results.push({
...
// =========================================================================
server.tool("detect_industry", "Detect the likely business industry for a domain using domain name keywords, TLD analysis, and homepage content scraping.", {
...
// =========================================================================
server.tool("search_new_businesses", "HIGH-LEVEL: Find new businesses by industry. Combines new domain registration search + industry detection + WHOIS enrichment. The main tool for lead generation — finds businesses 30-90 days before they appear on Google Maps.", {
    industry: z
        .string()
        .describe("Industry to search for (e.g. 'dental', 'hvac', 'law', 'plumbing', 'real_estate', 'restaurant')"),
    region: z
        .string()
        .optional()
        .describe("Filter by registrant region/country (e.g. 'US', 'United States', 'Alabama'). Checked against WHOIS registrant country/state."),
    days_back: z
        .number()
        .optional()
        ...
        for (const entry of newDomains) {
            ...
            try {
                ...
                if (region) {
                    ...
                    if (!country.includes(regionLower) &&
...
// =========================================================================
server.tool("domain_tech_check", "Detect technologies used by a domain's website. Checks for CMS, ecommerce, analytics, frameworks, and hosting via HTML pattern analysis.", {
...
// =========================================================================
server.tool("track_expirations", "Check WHOIS expiration dates for a list of domains. Useful for finding expiring competitor domains or acquisition opportunities.", {
    domains: z
        .array(z.string())
