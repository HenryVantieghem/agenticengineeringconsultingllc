...
export class IndustryDetector {
    ...
    // -------------------------------------------------------------------------
    matchDomainKeywords(domain, scores, signals) {
    ...
    // Signal 2: TLD analysis
    // -------------------------------------------------------------------------
    matchTLD(domain, scores, signals) {
    ...
    // -------------------------------------------------------------------------
    async scrapeHomepage(domain, scores, signals) {
        ...
        try {
            ...
            const descMatch = lowerHtml.match(/<meta[^>]*name=["']description["'][^>]*content=["'](.*?)["']/);
            ...
            for (const [industry, keywords] of Object.entries(INDUSTRY_KEYWORDS)) {
                for (const kw of keywords) {
                    if (combined.includes(kw)) {
                        scores[industry] = (scores[industry] ?? 0) + 30;
                        signals.push(`homepage_content: "${kw}" found in title/description -> ${industry}`);
...
//# sourceMappingURL=industry-detector.js.map