...
interface TechPattern {
  name: string;
  category: "cms" | "ecommerce" | "analytics" | "framework" | "hosting" | "other";
  patterns: RegExp[];
}
...
export class TechDetector {
   ...
   */
  async detect(domain: string): Promise<TechDetectionResult> {
      ...
      for (const pattern of tech.patterns) {
        if (pattern.test(html)) {
          ...
          switch (tech.category) {
            case "cms":
              if (!result.cms) result.cms = tech.name;
              break;
            case "ecommerce":
              if (!result.ecommerce) result.ecommerce = tech.name;
              break;
            case "analytics":
