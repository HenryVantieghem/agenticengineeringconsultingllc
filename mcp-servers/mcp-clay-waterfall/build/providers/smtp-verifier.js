 ...
 */
async function smtpCheck(mx, email) {
...
export class SmtpVerifierProvider {
    name = "smtp-verifier";
    isConfigured() {
    ...
    async findEmail(name, domain) {
        ...
        const candidates = EMAIL_PATTERNS.map((pattern) => pattern
            .replace("{first}", first)
            .replace("{last}", last)
            .replace("{f}", f) + `@${domain}`);
...
//# sourceMappingURL=smtp-verifier.js.map