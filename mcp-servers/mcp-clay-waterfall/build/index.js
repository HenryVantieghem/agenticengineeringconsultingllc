...
import { emailProviders, phoneProviders, personProviders, companyProviders, decisionMakerProviders, verificationProviders, allProviders, } from "./providers/index.js";
// ---------------------------------------------------------------------------
// Server init
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
server.tool("waterfall_find_email", "Find a person's email address by cascading through multiple data providers (Hunter, Snov, Apollo, Prospeo, RocketReach, PDL). Returns the first verified email found.", {
    ...
    const hit = await waterfall(emailProviders, (provider) => provider.findEmail(name, domain));
    if (!hit) {
        return {
            content: [
                {
...
// ---------------------------------------------------------------------------
server.tool("waterfall_find_phone", "Find a person's phone number by cascading through Lusha, Apollo, RocketReach, Snov, and PDL. Prefers mobile numbers.", {
    name: z.string().describe("Full name of the person"),
    company: z.string().describe("Company name"),
    domain: z.string().optional().describe("Company domain (optional)"),
}, async ({ name, company }) => {
    log("info", "waterfall_find_phone", { name, company });
    const hit = await waterfall(phoneProviders, (provider) => provider.findPhone ? provider.findPhone(name, company) : Promise.resolve(null));
    if (!hit) {
        return {
            content: [
                {
...
// ---------------------------------------------------------------------------
server.tool("enrich_company", "Enrich a company by domain — calls multiple providers in PARALLEL for comprehensive data (name, industry, employee count, revenue, tech stack, email pattern, social links).", {
    domain: z.string().describe("Company domain (e.g. 'acme.com')"),
}, async ({ domain }) => {
    log("info", "enrich_company", { domain });
    // Run all configured company providers in parallel, merge results
    const configured = companyProviders.filter((p) => p.isConfigured() && p.enrichCompany);
    const results = await Promise.allSettled(configured.map(async (provider) => {
        try {
    ...
    for (const settled of results) {
        ...
        for (const [key, val] of Object.entries(result.social_links)) {
            if (!merged.social_links[key])
                merged.social_links[key] = val;
        }
    }
    return {
        content: [
...
// ---------------------------------------------------------------------------
server.tool("enrich_person", "Enrich a person by email or name+company — cascades through Apollo, PDL, RocketReach, Prospeo, Snov, Hunter to find full profile (name, email, phone, title, LinkedIn, company, location).", {
    ...
    const hit = await waterfall(personProviders, (provider) => provider.enrichPerson
...
// ---------------------------------------------------------------------------
server.tool("find_decision_makers", "Find decision-makers at a company by title — searches Apollo, Hunter, RocketReach, PDL, and Snov for people matching the given titles.", {
    company: z.string().describe("Company name"),
    ...
    const hit = await waterfall(decisionMakerProviders, async (provider) => {
        if (!provider.findDecisionMakers)
            return null;
...
// Tool 6: verify_email
// ---------------------------------------------------------------------------
server.tool("verify_email", "Verify an email address using Hunter.io's verification endpoint. Returns validity status and deliverability score.", {
    email: z.string().describe("Email address to verify"),
}, async ({ email }) => {
    log("info", "verify_email", { email });
    const hit = await waterfall(verificationProviders, (provider) => provider.verifyEmail
...
// Tool 7: bulk_enrich
// ---------------------------------------------------------------------------
server.tool("bulk_enrich", "Bulk enrich a list of contacts — runs waterfall email finder + person enrichment for each contact. Rate-limited at 500ms between lookups to avoid API throttling.", {
    contacts: z
        .array(z.object({
    ...
    for (let i = 0; i < contacts.length; i++) {
        ...
        const emailHit = await waterfall(emailProviders, (provider) => provider.findEmail(contact.name, domain));
        if (emailHit) {
        ...
        const personHit = await waterfall(personProviders, (provider) => provider.enrichPerson
            ? provider.enrichPerson(identifier)
            : Promise.resolve(null));
        if (personHit) {
