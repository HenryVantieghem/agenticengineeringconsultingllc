...
// ---------------------------------------------------------------------------
export function calculateMonthlyMortgage(principal, annualRate, termYears) {
    if (principal <= 0)
        return 0;
    if (annualRate <= 0)
...
export function analyzeInvestment(input) {
    const { propertyValue, estimatedMonthlyRent, } = input;
    ...
    const totalMonthlyExpenses = Math.round((monthlyMortgage + monthlyTax + monthlyInsurance + monthlyMaintenance + monthlyVacancyReserve) * 100) / 100;
    // Cash flow
    ...
    const annualOperatingExpenses = (monthlyTax + monthlyInsurance + monthlyMaintenance + monthlyVacancyReserve) * 12;
...
//# sourceMappingURL=investment-calculator.js.map