...
server.tool(
  ...
  "Get comprehensive property details — bedrooms, bathrooms, sqft, lot size, year built, construction, heating/cooling, garage, pool, and more.",
  {
...
server.tool(
  "get_valuation",
  "Get property valuation — estimated market value (AVM), confidence score, price per sqft, last sale info, and value change trends.",
  {
...
server.tool(
  "get_owner_info",
  "Get property owner information — owner name, type (individual/corporate/trust), mailing address, owner-occupied status, purchase history.",
  {
    address: z.string().optional().describe("Street address"),
    city: z.string().optional().describe("City name"),
    state: z.string().optional().describe("State code"),
...
server.tool(
  ...
  "Find comparable property sales near a given address — sale price, date, sqft, price per sqft, distance, and similarity score.",
  {
...
server.tool(
  "search_properties",
  "Search for properties by location and criteria — filter by price range, property type, bedrooms, sqft. Returns property summaries with valuations.",
  {
...
server.tool(
  "get_rental_estimate",
  "Estimate monthly rental income for a property — estimated rent, range, cap rate, and gross yield. Works with address or property characteristics.",
  {
    address: z.string().optional().describe("Street address"),
    city: z.string().optional().describe("City name"),
    state: z.string().optional().describe("State code"),
...
server.tool(
  "analyze_investment",
  "Full investment analysis for a property — combines property value, rental estimate, and financing to calculate mortgage, cash flow, cap rate, cash-on-cash return, and break-even rent.",
  {
...
server.tool(
  "search_foreclosures",
  "Search for foreclosure and pre-foreclosure properties — find distressed properties including pre-foreclosure, auction, and REO (bank-owned).",
  {
    zipcode: z.string().optional().describe("ZIP code to search"),
    city: z.string().optional().describe("City name"),
    state: z.string().optional().describe("State code"),
